...
int32_t iodptcp_connect(IODP_TCP_TYPE* pDev, const char* ip, uint16_t port)
{
    ...
    pDev->serveraddr = server_addr;
    // 3、链接到服务器
    // ret = connect(pDev->sock_fd, (struct sockaddr *)&server_addr, sizeof(server_addr));
    // if (ret == SOCKET_ERROR){
    //     perror("connect");
    //     return -3;
    // }
    ...
    while (1)  
    {  
        retVal = connect(pDev->sock_fd, (struct sockaddr *)&server_addr, sizeof(server_addr));  
